...
<div class="person_itiran">
	...
	<ul class="ul">
		...
		<li><a href="/sfa/RegistListServlet?id=<%=i.getEmp_no()%>&name=<%=i.getName_kannji()%>"><%=i.getName_kannji()%></a></li><br>
		<%}%>
	</ul>
</div>
<div class="taiki-itiran">
	...
	<ul class="drag-list">
		...
		<a href="/sfa/RegistListServlet?id=<%=h.getEmp_no()%>&name=<%=h.getName_kannji()%>"><%=h.getName_kannji() %></a></li><br>
		<%} %>
	</ul>
</div>
<form action="/sfa/AnkenServlet" method="post">
	<div class="person_list">
		<!--<iframe src="/sfa/jsp/regist_list.jsp" width="240" height="400" class="list_box">
		</iframe> -->
		<div class="list_box">
			<ul>
				<%if(registList != null){ %>
					<%for(RegistList i : registList){ %>
					<li><a href="/sfa/DeleteListServlet?id=<%=i.getEmp_no()%>&name=<%=i.getName_kanji()%>">
		...
		<button type="button" onclick="location.href='/sfa/jsp/regist_anken.jsp'" class="btn">確定</button>
	</div>
</form>
</body>
</html>